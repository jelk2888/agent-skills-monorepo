...
// ============ 等待配置 ============
const WAIT_AFTER_DELETE = 5000;
const FIRST_CHECK_TIMEOUT = 30000;
const RETRY_CHECK_TIMEOUT = 30000;
...
function generateUserDataDir() {
  // 使用固定的用户数据目录，确保登录状态保持
  return path.join(__dirname, 'gemini-user-data');
}
...
async function batchGenerate(configPath, outputDir, options = {}) {
  const useHeadless = options.headless !== false;  // 默认 Headless
  ...
  console.log('\n🔄 正在关闭...');
  cdp.close();
  await chromeDebug.closeTab(targetId);
  chromeProcess.kill();

  // 保留用户数据目录，确保登录状态保持
  console.log('[Chrome] 保留用户数据目录，确保登录状态保持');
...
async function main() {
  ...
  if (args.length < 2) {
    ...
    console.log('║                                      ║');
    console.log('║  选项:                                ║');
    console.log('║    --headless     无窗口模式 (默认)     ║');
    console.log('║    --no-headless  弹出浏览器窗口        ║');
    console.log('║                                      ║');
    console.log('║  示例:                                ║');
