...
<p align="center">
  <img src="https://img.shields.io/badge/version-5.2-blue.svg" alt="Version">
...
- KaTeX 数学公式实时渲染
- 原理讲解（生动比喻，高中-大学水平）
- 可折叠小测验面板（可固定视口右下或挂在控制条右下锚点）
- 播放/暂停/单步/速度控制；理科 HTML 推荐「自动演示」（主参数循环）
- SVG 视口：滚轮缩放 + **左键/中键拖拽平移**；顶栏与控制条 **sticky** 便于长页课堂
...
### v5.2 (2026-05-12)
- ✅ SKILL：理科 HTML **课堂布局**（sticky 顶栏 + sticky 控制条、`body` 可纵向滚动）
- ✅ SVG 视口：**左键与中键**拖拽平移 + 滚轮缩放；`grab`/`grabbing` 光标约定
- ✅ 随堂测验：**方案 B** 控制条右下角锚点 + 下拉展开 + 折叠「?」FAB（与方案 A 并列写入技能）
- ✅ **自动演示**：`requestAnimationFrame` 驱动主时间参数；与重置/随机/手动滑块互斥停止
- ✅ 示例主题补充：**瓜豆原理**；参考落盘 `d:\HTML教学交互动画\瓜豆原理\index.html`
